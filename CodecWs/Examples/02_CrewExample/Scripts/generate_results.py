...
Automatically generate results.txt files for all QP encodings
"""
import os
import glob
from config import *

def generate_results(qp):
    """Generate results.txt for specific QP"""
    codec_dir = get_codec_dir(qp)
    bitstream = get_bitstream_file(qp)
    results_file = get_results_file(qp)
    
    # Check if bitstream exists
    if not os.path.exists(bitstream):
        print(f"WARNING: Bitstream not found for QP{qp}")
        return False
    ...
    results_content = f"""JSVM H.264/AVC - Crew CIF Example Results
============================================================
...
"""
    ...
    print(f"Generated results.txt for QP{qp}")
...
def main():
    """Generate results for all QP values"""
    ensure_dirs()
    
    # Find all QP directories
    codec_base = H264_CODEC_DIR
    qp_dirs = sorted(glob.glob(os.path.join(codec_base, "QP*")))
    
    print()
    print("=" * 70)
    print("Generating Results Files for All QP Values")
    print("=" * 70)
    print()
    ...
    if not qp_values:
        print("No QP directories found!")
        return 1
    
    # Sort QP values
    qp_values.sort()
    
    print(f"Found QP values: {qp_values}")
    print()
    
    # Generate results for each QP
    success_count = 0
    for qp in qp_values:
        if generate_results(qp):
            success_count += 1
        print()
    
    # Summary
    print("=" * 70)
    print(f"Successfully generated {success_count}/{len(qp_values)} results files")
    print("=" * 70)
